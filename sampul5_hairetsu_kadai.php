...
<html lang="ja">
	...
	<body>
		...
		<?php
		foreach ($students as $students_soeji => $soeji_value1) {
			print '<tr>
			 <td>'.$students[$students_soeji]['id'].'</td>
			 <td>'.$students[$students_soeji]['name'].'</td>
			 <td>'.$scores[$students[$students_soeji]['id']]['kokugo'].'</td>
			 <td>'.$scores[$students[$students_soeji]['id']]['sansu'].'</td>
			 <td>'.$scores[$students[$students_soeji]['id']]['rika'].'</td>
			 <td>'.$scores[$students[$students_soeji]['id']]['shakai'].'</td>';

			$sum = 0;
			foreach ($scores[$students[$students_soeji]['id']] as $key1 => $value1) {
					$sum = $sum + $value1;
			}
			print '<td>'.$sum.'</td>';
			$num_scores = count($scores[$students[$students_soeji]['id']]);
			$average = $sum / 4;
			print '<td>'.$average.'</td>';
			print '</tr>';
		}
